package j.com.problems;

/**
 * There is an integer array nums sorted in ascending order (with distinct values).
 * Prior to being passed to your function, nums is possibly rotated at an unknown pivot index k (1 <= k < nums.length)
 * such that the resulting array is [nums[k], nums[k+1], ..., nums[n-1], nums[0], nums[1], ..., nums[k-1]] (0-indexed).
 * For example, [0,1,2,4,5,6,7] might be rotated at pivot index 3 and become [4,5,6,7,0,1,2].
 * Given the array nums after the possible rotation and an integer target,
 * return the index of target if it is in nums, or -1 if it is not in nums.
 * You must write an algorithm with O(log n) runtime complexity.
 */
public class SearchInRotatedSortedArray {

    public static void main(String[] args) {
        testExpectedResult4();
        testExpectedResultNegative1();
        testExpectedResult0();
    }

    public static int findTargetIndex(int[] nums, int target) {
        //O(n)
        int left = 0;
        int right = nums.length - 1;

        while (left <= right) {
            if (nums[left] == target) {
                return left;
            }
            if (nums[right] == target) {
                return right;
            }
            left++;
            right--;
        }
        return -1;
    }

    public static int findTargetIndexRefined(int[] nums, int target) {
        //O(log n)
        int left = 0;
        int right = nums.length - 1;

        while (left <= right) {
            int mid = left + (right - left) / 2;
            if (nums[mid] == target) {
                return mid;
            }
            if (nums[left] <= nums[mid]) {
                if (nums[left] <= target && target < nums[mid]) {
                    right = mid - 1;
                } else {
                    left = mid + 1;
                }
            } else {
                if (nums[mid] <= target && target < nums[right]) {
                    left = mid + 1;
                } else {
                    right = mid - 1;
                }
            }
        }
        return -1;
    }

    private static void testExpectedResult4() {
        logResults(new int[]{4, 5, 6, 7, 0, 1, 2}, 0);
    }

    private static void testExpectedResultNegative1() {
        logResults(new int[]{4, 5, 6, 7, 0, 1, 2}, 3);
    }

    private static void testExpectedResult0() {
        logResults(new int[]{1}, 1);
    }

    private static void logResults(int[] nums, int target) {
        LoggerUtil.log(SearchInRotatedSortedArray.class, "findTargetIndex: " + findTargetIndex(nums, target),
                "findTargetIndexRefined: " + findTargetIndexRefined(nums, target));
    }
}
